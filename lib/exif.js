...
async function writeExifImg(media, metadata) {
    try {
        ...
        if (!fs.existsSync(tmpFileIn) || fs.statSync(tmpFileIn).size === 0) {
            throw new Error("File WebP yang dibuat kosong atau rusak.");
        }

        console.log("File WebP berhasil dibuat:", tmpFileIn);
        ...
        if (metadata.packname || metadata.author) {
            ...
            console.log("Exif berhasil ditambahkan:", tmpFileOut);
            return tmpFileOut;
        }
    } catch (error) {
        console.error("Error di writeExifImg:", error);
    }
}
