...
let handler = async (m, { zuu, isCreator, prefix, command, args }) => {
  zuu.saweria = zuu.saweria || "";
  zuu.gateway = zuu.gateway || [];

  const Pay = new Saweria(zuu.saweria);

  if (args[0] === "payment" || args[0] === "unban" || args[0] === "unblock") {
    const itemName = args[0].toUpperCase();
    const price = 1000;
    const pending = zuu.gateway.find(
      (v) => v.jid === m.sender && v.state === "PENDING",
    );
    const process = zuu.gateway.find(
    ...
    const gateway = zuu.gateway.find(
      (v) => v.jid === m.sender && v.state === "PENDING",
    );
    if (!gateway) return;
    ...
    if (pending) {
      zuu.gateway = removeItem(zuu.gateway, pending);
    }
    if (process) {
      zuu.gateway = removeItem(zuu.gateway, process);
    }
    ...
    m.reply(`Status Pembayaran: ✅ ${json.msg}`).then(() => {
      ...
      if (gateway.package === "PREMIUM") {
        ...
        zuu.updateBlockStatus(gateway.jid, "unblock");
      } else if (gateway.package === "DEPOSITO") {
        data.balance += gateway.amount;
      }

      zuu.gateway = removeItem(zuu.gateway, gateway);
    });
