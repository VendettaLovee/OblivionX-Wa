...
const jadibot = async (zuu, text, m, from, type) => {
    ...
    try {
        async function startBotz() {
            ...
            const zuu = await simple.makeWASocket(config)

            /* ━━━━━━━━━[ --------- ]━━━━━━━━━ */

            if (type === "pairing") {
                if (!zuu.authState.creds.registered) {
                    setTimeout(async () => {
                        let phoneNumber = `${text}`
                        console.log(phoneNumber)
                        let code = await zuu.requestPairingCode(phoneNumber)
            ...
            store.bind(zuu.ev);
            zuu.ev.on('messages.update', async (chatUpdate) => {
                pollup(chatUpdate, store, zuu)
            })
            zuu.ev.on('connection.update', async (update) => {
                if (type === "scan") {
                    console.log(update)
                ...
                pathcer.connection(update, startBotz, zuu, usePairingCode, type, from, m)
            });
            zuu.ev.on('contacts.update', (update) => {
                saver(update, store, zuu)
            })
            zuu.ev.on("messages.upsert", async (chatUpdate) => {
                if (chatUpdate.type === 'notify') {
                    await new Promise(resolve => setTimeout(resolve, 500));
                    tocase(chatUpdate, store, zuu);
                } else {
                    tocase(chatUpdate, store, zuu)
                }
            });
            ...
            zuu.ev.on("creds.update", saveCreds);
