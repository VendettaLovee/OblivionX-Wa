...
await chek._quickTest()
 ...
 await chek.version()
    ...
    if (!zuu.authState.creds.registered && usePairingCode) {
        verifing("pairing")
        const phoneNumber = await question("     ");
        const code = await zuu.requestPairingCode(phoneNumber.replace(/[-+ ]/g, ''))
    ...
    zuu.ev.on('connection.update', async (update) => {
        pathcer.connection(update, startBotz, zuu, usePairingCode)
    });
