...
async function catbox(file) {
    try {
        const form = new BodyForm();
        const rstream = await fs.createReadStream(file)
        form.append('fileToUpload', rstream);
...
async function wikimon(digimon) {
    try {

        const url = `https://digimon.fandom.com/wiki/${digimon}`
        const {
            data
        } = await axios.get(url, {
            timeout: 10000
        ...
        if (!name) return "Gada digimon kaya gitu";
        $('.pi-data-value.pi-font').each((index, element) => {
            statuses.push($(element).text().trim());
        });

        let Selfie;
        let Desc = $('meta[property="og:description"]').attr('content');
        Selfie = $('meta[property="og:image"]').attr('content');

        if (!Selfie) {
            Selfie = "https://files.catbox.moe/n54ddz.jpg"
        };

        const q = (ArrayNum) => {
            return statuses[ArrayNum];
        }
