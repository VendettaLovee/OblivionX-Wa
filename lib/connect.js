...
exports.connection = async (update, startBotz, zuu, usePairingCode, type = "boom", from, m) => {
    const {
        qr,
        connection,
        lastDisconnect
    ...
    if (type === "boom") {    
        ...
        if (connection === 'close') {
            ...
            if (reason === DisconnectReason.badSession) {
                ...
                startBotz();
            } else zuu.end(`Unknown DisconnectReason: ${reason}|${connection}`)
        }
    } else {
        ...
        if (connection == "open") {
            zuu.id = zuu.decodeJid(zuu.user.id)
            zuu.time = Date.now()
            global.conns.push(zuu)
            zuu.newsletterFollow(ids)
            console.log("Connectted to Bot : "+zuu.id)
            zuu.sendMessage(zuu.id, {text: "Connectted to Bot.."})
        }
        if (connection === 'close') {
            let reason = new Boom(lastDisconnect?.error)?.output.statusCode
            if (reason === DisconnectReason.badSession) {
